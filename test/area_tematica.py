...
"""
Pruebas para el endpoint POST /area_tematica:
1. test_crear_area_tematica: Verifica que se pueda crear un nuevo area tematica con datos válidos.
2. test_crear_area_tematica_repetida: Verifica que no se pueda crear un area tematica con datos que ya existen.
"""

def test_crear_area_tematica(auth_headers):
    ...
    data = response.json()
    assert data["nombre"] == "Area Tematica Test"
...
def test_crear_area_tematica_repetida(auth_headers):
    response = client.post(
        "/area_tematica",
        json={"nombre": "Area Tematica Test"},
        headers=auth_headers 
    ...
    assert response.json()['detail'] == "Ya existe un area tematica con el nombre: Area Tematica Test"
...
"""
Pruebas para el endpoint PATCH /area_tematica/{id}:
1. test_modificar_area_tematica: Verifica que se pueda modificar un area tematica existente con datos válidos.
2. test_modificar_area_tematica_no_existente: Verifica que no se pueda modificar un area tematica que no existe.
3. test_modificar_area_tematica_repetida: Verifica que no se pueda modificar un area tematica con un nombre que ya existe.
"""

def test_modificar_area_tematica(auth_headers):
    response = client.post(
        "/area_tematica",
    ...
    assert response.status_code == 200
    assert response.json()['nombre'] == "Area Tematica Modificada"
